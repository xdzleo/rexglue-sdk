...
X_STATUS XTimer::SetTimer(int64_t due_time, uint32_t period_ms, uint32_t routine,
  ...
  // resume asks us to wake the console out of suspend when this timer fires,
  // which we cannot do -- but returning up front (as we used to) meant the
  // timer was never armed at all, so the periodic APC never fired and the
  // title sat waiting on a tick that could not arrive. Arm it first, then
  // report the resume request as ignored; the guest sees the same status it
  // always did. Adopted from Xenia Canary d680f5a499.
  if (resume) {
