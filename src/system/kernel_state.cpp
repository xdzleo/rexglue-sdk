...
object_ref<UserModule> KernelState::LoadUserModule(const std::string_view raw_name,
  ...
  if (recomp && !recomp->shared_lib_name.empty()) {
    ...
    if (!library_local.Load(recomp->shared_lib_name)) {
      ...
      // GetSymbol, not their GetRawSymbol: that name belongs to
      // rex::platform::DynamicLibrary, which system::SharedLibrary is only
      // collapsed onto by the 0.9.0 platform refactor we did not take.
      using GetImageInfoFn = const rex::PPCImageInfo* (*)();
      auto get_image_info =
          reinterpret_cast<GetImageInfoFn>(library_local.GetSymbol("ReXModule_GetImageInfo"));
      if (!register_func) {
